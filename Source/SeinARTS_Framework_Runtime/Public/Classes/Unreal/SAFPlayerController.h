...
public:
  ...
  FVector LineTraceUnderScreenPosition(FVector2D ScreenPosition, float Distance, TEnumAsByte<ECollisionChannel> TraceChannel, bool bTraceComplex, UPARAM(ref) AActor*& OutActor, bool bPrintDebugMessages = true) const;

  // Line trace from mouse cursor into the world; returns hit location (or ZeroVector if no hit).
  UFUNCTION(BlueprintCallable, Category="SeinARTS|Input")
  FVector LineTraceUnderCursor(UPARAM(ref) AActor*& OutActor, bool bPrintDebugMessages = true) const { return LineTraceUnderScreenPosition(GetMouseVector2D(), TraceDistance, CursorTraceChannel, false, OutActor, bPrintDebugMessages); }
...
protected:
  ...
  // Replication
	// =======================================================================================================
  // Called by the client to notify readiness to start the match.
