...
namespace SAFLibrary {
  ...
  bool IsActorPtrValidSeinARTSActor(AActor* InPtr) {
    return IsValid(InPtr) 
    && InPtr->GetClass()->ImplementsInterface(USAFAssetInterface::StaticClass()) 
    && !InPtr->IsActorBeingDestroyed();
  }
  ...
  /** Decide if a candidate actor is friendly to a requesting player. */
    ...
    if (IsActorPtrValidSeinARTSActor(Actor) && ISAFAssetInterface::Execute_GetOwningPlayer(Actor) != RequestingPlayer) return false;
    return true;
	}
  ...
  /** Filter input actors by player friendliness into OutActors. */
    ...
    for (AActor* Actor : InActors) {
      ...
      ASAFPlayerState* Owner = ISAFAssetInterface::Execute_GetOwningPlayer(Actor);
