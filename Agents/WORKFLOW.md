...
This is the local operational mirror of the human [Workflow Policy](https://docs.google.com/document/d/1pb3Z0DdQKAIJ610cMOy1yOP9_RQj1jtzMupfhkyrlfw). The human policy owns contributor workflow. This file contains the rules agents must apply while working in this repository. Update both in the same task when workflow changes.
...
Branches and worktrees belong to tasks, not contributors. Use a separate branch and worktree for independent writing or implementation tasks. Do not develop unrelated work directly on `main`.
