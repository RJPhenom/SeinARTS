...
> picker; the `FSeinPath` typed-segment seam). Command-broker idle re-seek and the A* non-shipping
> path reporters are now isolated behind private implementation boundaries with focused coverage;
> the remaining maintainability target is the avoidance kernel, without redesigning its seam, plus
> qualification of Movement+'s shipped steering-first, curated
