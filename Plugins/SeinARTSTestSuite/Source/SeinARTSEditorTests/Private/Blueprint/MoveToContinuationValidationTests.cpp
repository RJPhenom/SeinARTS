...
namespace UE::SeinARTSTests::MoveToContinuationValidation
{
	...
	UK2Node_AsyncAction* AddValidationAsyncAfter(
		FFixture& Fixture,
	...
	"SeinARTS.Editor.Blueprint.MoveToContinuation")
{
	...
	UK2Node_AsyncAction* Nested = AddValidationAsyncAfter(
		Fixture,
		*Fixture.MoveTo->FindPinChecked(
			GET_MEMBER_NAME_CHECKED(
				USeinMoveToProxy,
	...
	"SeinARTS.Editor.Blueprint.MoveToContinuation")
{
	...
	UK2Node_AsyncAction* First = AddValidationAsyncAfter(
		Fixture,
		*Independent->FindPinChecked(
			UEdGraphSchema_K2::PN_Then));
	ASSERT_THAT(IsNotNull(First));
	UK2Node_AsyncAction* Second = AddValidationAsyncAfter(
		Fixture,
