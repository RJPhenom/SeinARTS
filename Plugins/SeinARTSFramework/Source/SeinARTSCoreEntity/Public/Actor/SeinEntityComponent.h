...
public:
	 ...
	 *       when UE's chain propagation actually reaches here. The chain
	 *       only includes the parent ComponentData array for some edit
	 *       shapes (direct field edits in the picker UI); custom struct
	 *       customizations that fire `NotifyPostChange` on the inner
	 *       struct handle land without ComponentData in the chain, so
	 *       they explicitly call `PropagateComponentDataEntryToInstances`
	 *       themselves. See FSeinCoverComponentDetails::OnGenerateButtonClicked
	 *       for an example. */
	 ...
	 *  entire entry.
	 *
	 *  Public so struct customizations (cover Generate Slots, future
	 *  per-component buttons) can fire it directly after mutation —
	 *  avoids relying on UE's chain-propagation reaching the bridge with
	 *  ComponentData visible in the chain. */
