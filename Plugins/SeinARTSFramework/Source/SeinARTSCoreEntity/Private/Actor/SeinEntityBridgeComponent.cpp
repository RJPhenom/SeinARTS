...
USeinEntityBridgeComponent::USeinEntityBridgeComponent()
{
...
bool USeinEntityBridgeComponent::ValidateComponentData(
...
void USeinEntityBridgeComponent::InjectAuthoredComponents(USeinWorldSubsystem& World, FSeinEntityHandle Handle) const
{
	...
	// Auto-inject the universal FSeinFogVisibilityPayload from the bridge's
	// top-level fields. Authoring lives on USeinEntityBridgeComponent directly
	...
	{
		FSeinFogVisibilityPayload FogVis;
		FogVis.FogVisibilityPolicy = FogVisibilityPolicy;
		FogVis.FogVisibilityLayerMask = FogVisibilityLayerMask;
		ISeinComponentStorage* FogVisStorage = World.GetOrCreateStorageForType(FSeinFogVisibilityPayload::StaticStruct());
		if (FogVisStorage)
		{
...
void USeinEntityBridgeComponent::BeginPlay()
{
	Super::BeginPlay();
...
void USeinEntityBridgeComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
namespace
{
	...
	bool IsClassDefaultBridge(const USeinEntityBridgeComponent& Bridge)
	{
	...
	const USeinEntityBridgeComponent* FindInheritedDefaultBridge(
		const USeinEntityBridgeComponent& Source)
	{
		if (Source.IsTemplate())
		{
			const USeinEntityBridgeComponent* Parent =
				Cast<USeinEntityBridgeComponent>(Source.GetArchetype());
		...
		const AActor* ActorCDO = OwnerClass
			...
			: nullptr;
		const USeinEntityBridgeComponent* ClassDefault = ActorCDO
			? ActorCDO->FindComponentByClass<USeinEntityBridgeComponent>()
			: nullptr;
	...
	bool ImportPatchValue(
		USeinEntityBridgeComponent& Target,
	...
	bool DoesPatchEqualComponentValue(
		const FSeinComponentPropertyPatch& Patch,
		const USeinEntityBridgeComponent& Left,
		const USeinEntityBridgeComponent& Right)
	{
	...
	bool DoesPatchEqualExportedValue(
		const FSeinComponentPropertyPatch& Patch,
		const USeinEntityBridgeComponent& Component)
	{
		...
		const TCHAR* End = CandidateProperty->ImportText_Direct(
			*Patch.ExportedValue, CandidateValue,
			const_cast<USeinEntityBridgeComponent*>(&Component), PPF_None);
		if (!End) return false;
...
void USeinEntityBridgeComponent::PostInitProperties()
{
...
void USeinEntityBridgeComponent::EnsureComponentDataOverrideMetadataInitialized()
{
	if (IsTemplate())
	{
		return;
	...
	const USeinEntityBridgeComponent* ClassDefault =
...
void USeinEntityBridgeComponent::RecordComponentDataClassDefaultChange(
...
void USeinEntityBridgeComponent::CatchUpComponentDataClassDefaultHistory()
{
	const USeinEntityBridgeComponent* Ancestor = FindInheritedDefaultBridge(*this);
	if (!Ancestor
...
void USeinEntityBridgeComponent::RefreshInheritedComponentDataFromClassDefaults()
{
...
void USeinEntityBridgeComponent::PostLoad()
{
	Super::PostLoad();
	if (IsTemplate())
	{
...
void USeinEntityBridgeComponent::BeginComponentDataEdit()
{
	ComponentDataBeforeEditorChange = ComponentData;
	bCapturedComponentDataBeforeEditorChange = true;
}

void USeinEntityBridgeComponent::EndComponentDataEdit()
{
	HandleComponentDataEdited();
}

void USeinEntityBridgeComponent::PreEditChange(FEditPropertyChain& PropertyAboutToChange)
{
	...
	for (auto Node = PropertyAboutToChange.GetHead(); Node; Node = Node->GetNextNode())
	{
		...
		if (Property
			&& Property->GetFName()
				== GET_MEMBER_NAME_CHECKED(USeinEntityBridgeComponent, ComponentData))
		{
...
void USeinEntityBridgeComponent::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	for (auto Node = PropertyChangedEvent.PropertyChain.GetHead(); Node; Node = Node->GetNextNode())
	{
		...
		if (Prop && Prop->GetFName() == GET_MEMBER_NAME_CHECKED(USeinEntityBridgeComponent, ComponentData))
		{
			ArrayProp = Prop;
	...
	// Watch for designer edits to `FSeinIdentityPayload::IdentityTag` inside
	...
	const FName ChangedLeaf = PropertyChangedEvent.Property
		...
		: NAME_None;
	if (ChangedLeaf == GET_MEMBER_NAME_CHECKED(FSeinIdentityPayload, IdentityTag)
		&& ArrayIndex != INDEX_NONE && ComponentData.IsValidIndex(ArrayIndex))
	{
		FInstancedStruct& Entry = ComponentData[ArrayIndex];
		if (Entry.IsValid() && Entry.GetScriptStruct() == FSeinIdentityPayload::StaticStruct())
		{
...
void USeinEntityBridgeComponent::HandleComponentDataEdited()
{
	...
	EnsureComponentDataOverrideMetadataInitialized();
	const USeinEntityBridgeComponent* ClassDefault = FindInheritedDefaultBridge(*this);
	if (ClassDefault)
	{
...
void USeinEntityBridgeComponent::PropagateComponentDataChangesToInstances(
	...
	TArray<FSeinComponentLiveTuningClassEntry>& OutDerivedClassEntries)
{
	...
	for (UObject* InstanceObj : ArchetypeInstances)
	{
		...
		TArray<USeinEntityBridgeComponent*> InstBridges;
		Instance->GetComponents<USeinEntityBridgeComponent>(InstBridges);
		for (USeinEntityBridgeComponent* InstBridge : InstBridges)
		{
...
bool USeinEntityBridgeComponent::RecordStructuralChangeToOwnHistory(
...
USeinEntityBridgeComponent::EStructuralAdoptOutcome
USeinEntityBridgeComponent::TryAdoptStructuralRecord(
...
bool USeinEntityBridgeComponent::ReconcileStructuralOverrideFlag()
{
	if (IsTemplate())
	{
		return false;
...
void USeinEntityBridgeComponent::HandleStructuralClassDefaultEdit()
{
...
bool USeinEntityBridgeComponent::IsComponentDataShapeExplainedByAuthoring() const
{
	if (IsTemplate())
	{
		return false;
...
void USeinEntityBridgeComponent::BakeAuthoredDataComponents(bool bInteractive)
{
...
void USeinEntityBridgeComponent::PreSave(FObjectPreSaveContext SaveContext)
{
...
void USeinEntityBridgeComponent::SetEntityHandle(FSeinEntityHandle InHandle)
{
...
bool USeinEntityBridgeComponent::HasValidEntity() const
{
	...
	USeinWorldSubsystem* Subsystem = const_cast<USeinEntityBridgeComponent*>(this)->GetSubsystem();
	if (!Subsystem)
	{
		return false;
	}
...
void USeinEntityBridgeComponent::SetTransformSyncEnabled(bool bEnable)
{
	bSyncTransform = bEnable;
	SetComponentTickEnabled(bEnable);
}

void USeinEntityBridgeComponent::ApplyRayTracingGeometryPolicy()
{
...
void USeinEntityBridgeComponent::ApplySkeletalMeshPerformancePolicy()
{
...
void USeinEntityBridgeComponent::OnSimFrame(int32 TicksProcessed)
{
	USeinWorldSubsystem* Subsystem = GetSubsystem();
...
USeinWorldSubsystem* USeinEntityBridgeComponent::GetSubsystem()
{
	if (!CachedSubsystem)
	{
...
void USeinEntityBridgeComponent::SyncTransformToActor()
{
	USeinWorldSubsystem* Subsystem = GetSubsystem();
	if (!Subsystem)
	{
