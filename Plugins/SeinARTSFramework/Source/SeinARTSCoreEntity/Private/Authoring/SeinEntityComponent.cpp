 ...
 * @created      7 Sep 2026
 * @latest       7 Sep 2026
 * @brief        Implements data-only authoring and preserves manual identity ownership during baking.
...
{
    if (auto* Identity = Cast<USeinIdentityComponent>(this))
    {
        const FName Changed = PropertyChangedEvent.GetPropertyName();
        if (Changed == GET_MEMBER_NAME_CHECKED(FSeinIdentityPayload, IdentityTag)
            || Changed == GET_MEMBER_NAME_CHECKED(USeinIdentityComponent, Identity))
            Identity->Identity.bAutoGeneratedTag = false;
    }
