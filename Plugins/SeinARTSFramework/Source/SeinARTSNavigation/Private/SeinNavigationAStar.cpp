...
#include "SeinNavigationAStarAsset.h"
...
bool USeinNavigationAStar::BeginBake(UWorld* World)
{
	if (!World)
	{
		UE_LOG(LogSeinNavigationAStar, Warning, TEXT("BeginBake: null world"));
		return false;
	}
	if (bBaking)
	{
		UE_LOG(LogSeinNavigationAStar, Warning, TEXT("BeginBake: already baking"));
		return false;
	}
	...
	if (!bOk || !NewAsset)
	{
		UE_LOG(LogSeinNavigationAStar, Warning, TEXT("BeginBake: failed"));
		return false;
	}
	...
	UE_LOG(LogSeinNavigationAStar, Log, TEXT("Bake complete: %dx%d cells, CellSize=%s"),
...
bool USeinNavigationAStar::DoSyncBake(UWorld* World, USeinNavigationAStarAsset*& OutAsset)
{
	...
	{
		UE_LOG(LogSeinNavigationAStar, Warning, TEXT("DoSyncBake: no NavVolumes in world"));
		return false;
	}
...
#if WITH_EDITOR
	FScopedSlowTask Task(GridW * GridH, NSLOCTEXT("SeinNavigationAStar", "Baking", "Baking SeinARTS A* Nav..."));
	...
	// Per-entity-class skip list. Two reasons to exclude an ASeinActor's geometry
	// from the static bake:
	//   1) Designer flagged the entity class as `bBakesIntoNav = false` (e.g. a
	//      decorative prop, a lampshade, an entity that should be walkable
	//      through, an abstract entity with editor-only preview meshes). Hard
	//      override.
	...
	//      on a non-mobile entity class, or simply not give it a movement
	//      component.
	...
	for (TActorIterator<ASeinActor> It(World); It; ++It)
	{
		...
		// Hard-override: FSeinExtentsComponent::bBakesIntoNav. When the
		// entity bridge carries an extents entry with bBakesIntoNav=false
		// the entity class opts out of static nav baking.
		...
		// Mobile heuristic — any FSeinMovementComponent authored in the
		// entity bridge's ComponentData array means the entity class is
		// movement-driven, so its at-bake pose is irrelevant. Post-Phase-5
		// the legacy USeinMovementComponent AC is gone; mobility lives on
		// the entity bridge.
		if (!bSkip)
		{
			if (const USeinEntityComponent* Bridge = SeinActor->FindComponentByClass<USeinEntityComponent>())
	...
	for (int32 Y = 0; Y < GridH; ++Y)
	{
		for (int32 X = 0; X < GridW; ++X)
		{
			if (bCancelRequested)
			{
				UE_LOG(LogSeinNavigationAStar, Warning, TEXT("Bake cancelled by user"));
				OutAsset = nullptr;
				return false;
			}
	...
	// ========================================================================
	{
		...
		UE_LOG(LogSeinNavigationAStar, Log, TEXT("Bake connectivity: edges=%d blocked_edges=%d FallbackStep=%.1f"),
	...
	// blocked. Without this, GetCellHeightAt(walkable-only) on a cube-footprint cell would
	...
	// ========================================================================
	{
		...
		// ground, or whether a stray connection bit merged them.
		{
			...
			UE_LOG(LogSeinNavigationAStar, Log, TEXT("Bake component sizes: %s"), *Sizes);
		}
	...
	{
		UE_LOG(LogSeinNavigationAStar, Warning, TEXT("Bake: failed to save asset to disk"));
...
{
	// Resolve save folder from plugin settings. Default `/Game/NavData` only
	// when the path is empty or clearly not a content-mount path. UE content
	// mounts include `/Game/` (project), `/Engine/` (engine), and any plugin
	// name (e.g. `/SeinARTSFramework/...` for assets under this plugin's
	// `Content/`). Accept anything starting with `/` — the content browser
	// picker enforces valid mount points; the loosened check just avoids
	// double-validating something UE already validated.
	//
	...
	const USeinARTSCoreSettings* Settings = GetDefault<USeinARTSCoreSettings>();
	FString SaveFolder = (Settings ? Settings->NavDataSaveFolder.Path : FString());
	if (SaveFolder.IsEmpty() || !SaveFolder.StartsWith(TEXT("/")))
	{
		SaveFolder = TEXT("/Game/NavData");
	}
