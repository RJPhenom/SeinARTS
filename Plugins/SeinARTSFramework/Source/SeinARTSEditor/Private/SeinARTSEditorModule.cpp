...
{
    /** The registry callback runs before AssetTools saves the renamed owner. */
    static void OnAssetRenamed(const FAssetData& Asset, const FString& OldObjectPath)
    {
        if (!GetDefault<USeinARTSCoreSettings>()->bEnableAutoTagGeneration) return;
        UBlueprint* BP = Cast<UBlueprint>(Asset.FastGetAsset(false));
        if (!BP || FPackageName::ObjectPathToObjectName(OldObjectPath) == BP->GetName()) return;
        FGameplayTag Current;
        bool bGenerated = false;
        if (!SeinAutoTag::ReadAssetIdentity(BP, Current, bGenerated) || !bGenerated) return;
        static bool bMigrating = false;
        if (bMigrating) return;
        TGuardValue<bool> Guard(bMigrating, true);
        const auto Result = SeinAutoTag::ReconcileGeneratedIdentity(BP,
            Current.IsValid() ? Current.GetTagName() : SeinAutoTag::LastGeneratedIdentity(BP));
        if (Result.IsFailure())
        {
            UE_LOG(LogSeinAutoTag, Warning, TEXT("%s"), *Result.ToUserMessage(BP).ToString());
    ...
    {
        // Registry discovery and loading existing packages must never rewrite identities.
        // AssetCreated for a freshly duplicated/created Blueprint runs after its CDO exists.
        UBlueprint* BP = Cast<UBlueprint>(Asset.FastGetAsset(false));
        if (!IsValid(BP) || BP->HasAnyFlags(RF_Transient)) return;
        if (!FPackageName::IsValidLongPackageName(Asset.PackageName.ToString())
            || FPackageName::DoesPackageExist(Asset.PackageName.ToString())) return;
        static bool bAssigning = false;
        if (bAssigning) return;
        TGuardValue<bool> Guard(bAssigning, true);
        FGameplayTag Current;
        bool bGenerated = false;
        if (!SeinAutoTag::ReadAssetIdentity(BP, Current, bGenerated) || !bGenerated) return;
        const FName Proposed = SeinAutoTag::ProposeTagName(BP->GetFName(), GetDefault<USeinARTSCoreSettings>());
        if (Proposed.IsNone() || Current.GetTagName() == Proposed) return;
        const auto Result = SeinAutoTag::InitializeNewAssetTag(BP);
        if (Result.IsFailure()) UE_LOG(LogSeinAutoTag, Warning, TEXT("%s"), *Result.ToUserMessage(BP).ToString());
    }
...
namespace SeinAbilityContinuationCompilerGate
{
	...
	void Validate(UBlueprint* Blueprint)
	{
		...
		// Skeleton properties reflect the declaration being compiled, including
		// newly added variables that are not in the previous generated class.
		FString InputError;
		if (Blueprint->SkeletonGeneratedClass && !FSeinAbilityActivationInputs::ValidateClass(
			Blueprint->SkeletonGeneratedClass, InputError))
			Blueprint->Message_Error(InputError);
...
void FSeinARTSEditorModule::StartupModule()
{
	...
	{
		ActivationVariableCustomizationHandle = FModuleManager::LoadModuleChecked<FBlueprintEditorModule>(TEXT("Kismet")).RegisterVariableCustomization(
			FProperty::StaticClass(), FOnGetVariableCustomizationInstance::CreateLambda([](TSharedPtr<IBlueprintEditor> Editor)
			{ return MakeShared<FSeinAbilityVariableDetails>(Editor); }));
	}
	...
	SeinComponentDeletion::Register();
	if (GEditor != nullptr)
	{
	...
	// New copies receive distinct identities; renaming an auto-owned asset migrates its tag
	// hierarchy and references before the renamed owner is saved.
	{
		FAssetRegistryModule& AssetRegistryModule =
			FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
		OnTaggableAssetAddedHandle = AssetRegistryModule.Get().OnAssetAdded().AddStatic(&SeinAutoTagRenameHandler::OnAssetAdded);
...
{
	SeinAutoTag::ShutdownMigrationSupport();
	ReleaseModuleOwnedState();
}
...
void FSeinARTSEditorModule::ReleaseModuleOwnedState()
{
	...
	if (FBlueprintEditorModule* Kismet = FModuleManager::GetModulePtr<FBlueprintEditorModule>(TEXT("Kismet")))
		Kismet->UnregisterVariableCustomization(FProperty::StaticClass(), ActivationVariableCustomizationHandle);
	ActivationVariableCustomizationHandle.Reset();
	...
	SeinComponentDeletion::Unregister();
	...
	if (OnAssetRenamedHandle.IsValid())
	{
				...
				FModuleManager::GetModulePtr<FAssetRegistryModule>("AssetRegistry"))
		{
			...
			AssetRegistryModule->Get().OnAssetAdded().Remove(OnTaggableAssetAddedHandle);
		}
