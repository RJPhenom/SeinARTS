 ...
 * @created      7 Sep 2026
 * @latest       7 Sep 2026
 * @brief        Creates entity Blueprints with authoritative identity authoring components.
...
UObject* USeinEntityFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn, FName CallingContext)
{
    ...
    if (NewBP && NewBP->GeneratedClass && !ProposedTag.IsNone())
    {
        TArray<const USeinIdentityComponent*> Identities;
        AActor::GetActorClassDefaultComponents<USeinIdentityComponent>(NewBP->GeneratedClass.Get(), Identities);
        if (Identities.IsEmpty() && NewBP->SimpleConstructionScript)
        {
            NewBP->SimpleConstructionScript->AddNode(NewBP->SimpleConstructionScript->CreateNode(
                USeinIdentityComponent::StaticClass(), TEXT("SeinIdentity")));
