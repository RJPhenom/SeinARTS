...
void FSeinARTSCoreSettingsDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	// Saved evidence is an advanced recovery/testing tool. Ordinary Play and
	// cook manage compatibility without this source asset.
	{
		...
		[
			...
			.ToolTipText(
				LOCTEXT(
					...
					"Rebuilds optional saved evidence for strict compatibility testing or recovery. Ordinary Play does not need this action. Packaging generates compatibility data automatically. Compile and save source assets before using this tool."))
		]
		.ValueContent()
    ...
    TagGroup.AddWidgetRow().FilterString(LOCTEXT("TagAuditFilter", "Generated Tags Cleanup"))
    .WholeRowContent()
    [
        SNew(SButton)
        .Text(LOCTEXT("TagAuditButton", "Inspect and Clean Up Generated Tags"))
        .ToolTipText(LOCTEXT("TagAuditTip", "See generated tags, their owners and retention reasons. Cleanup removes only unused generated entries after content is saved. Asset renames keep their existing identities."))
        .OnClicked_Lambda([]() { SeinTagAudit::ShowWindow(); return FReply::Handled(); })
    ];
