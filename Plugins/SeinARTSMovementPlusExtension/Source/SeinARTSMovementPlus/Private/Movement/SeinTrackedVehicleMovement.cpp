	...
	FSeinMovementRenderStateWriter& Writer) const
{
	...
	const FSeinTrackedMovementData* Tracked =
		MovementData.MovementClassData.GetPtr<FSeinTrackedMovementData>();
	if (!Tracked || !Context.bHasPreviousSample
	...
	const FFixedPoint AccelDenom =
		Tracked->Acceleration * Context.DeltaTime;
	const FFixedPoint DecelDenom =
		Tracked->Deceleration * Context.DeltaTime;
	...
	Writer.SetValue(SteeringAngleSlot, FFixedPoint::Zero);
