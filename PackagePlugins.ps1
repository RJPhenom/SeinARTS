...
.DESCRIPTION
  ...
  notify-Consumer workflow fires on the published release and dispatches
  Consumer's seinarts-update workflow, which installs the packages into
  Consumer/Plugins/ and commits. Your side of the loop: run this, then
  `git pull` in Consumer.
...
Published releases are installed into Consumer automatically by its seinarts-update workflow.
"@
...
# Kick Consumer's install workflow directly — your gh login already spans both
...
if ($LASTEXITCODE -ne 0) {
    Write-Warning "Release v$Version published, but triggering Consumer's workflow failed. Run it manually from Consumer's Actions tab with tag 'v$Version'."
} else {
    Write-Host "[PackagePlugins] release v$Version published; Consumer's seinarts-update workflow is running. 'git pull' Consumer once it commits." -ForegroundColor Green
}
